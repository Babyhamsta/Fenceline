...
writeFileSync(
  ...
  [
    "badsite.example",
    "www.alsobad.example",
    "sub.deep.bad.example",
    "# comment",
    "0.0.0.0 hostsstyle.example",
    "REMOVED.allowed.example"
  ].join("\n")
);
...
assert(
  tailBuf.length === meta.tail.count * 8 && catsBuf.length === meta.tail.count,
  "artifact sizes consistent"
);
...
assert(
  check("badsite.example")?.category === "adult",
  "exact match -> adult (dupe precedence over gambling)"
);
...
assert(
  check("districtblocked.example")?.category === "custom",
  "lists/block.txt -> custom category"
);
...
for (const chunk of meta.chunks) {
  ...
  assert(
    chunk.ruleIdStart === expectedStart,
    `${chunk.file} ruleIdStart ${chunk.ruleIdStart} == expected ${expectedStart}`
  );
  assert(
    rules.length === chunk.ruleCount && rules.length <= chunk.maxRules,
    `${chunk.file} ruleCount within reserved range`
  );
  for (const r of rules) {
    const size = JSON.stringify(r).length;
    assert(
      r.action.type === "block" &&
        Array.isArray(r.condition.requestDomains) &&
        r.condition.requestDomains.length <= 3 &&
        size <= 1900,
...
assert(
  totalRuleDomains === meta.counts.dnrTier,
  `DNR tier domain count consistent (${totalRuleDomains})`
);
